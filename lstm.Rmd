...
## XXX added reading of df_watch.rds here
df_watch <- read_rds("df_watch.rds")

df_watch$target_var <- as.logical("FALSE")
df_watch[which(df_watch$date%like%"12-31"),]$target_var <- as.logical("TRUE")
df_watch[which(df_watch$date%like%"01-01"),]$target_var <- as.logical("TRUE")
df <- df_watch
```

```{r}
# get a smaller dataset and add a new column named species by the year of that species exist on the site. 
selected_sites <- unique(df_phenology$id_site)[1:4]
df_withspecies <- c()
...
vars <- c("sitename","rain","snow","prec","qair","temp","patm","vapr","vpd","target_var")
vars_meta <- c("species")

#one-hot encoding of the variable 
meta_df <- df_withspecies%>%select(vars_meta)
meta_df <- get_dummies.(data.frame(meta_df))

#seperating two dataframe
meta_df$site <- df_withspecies$sitename
sensor_df <- df_withspecies%>%select(vars)
...
# normalisaton
myrecipe_ddf <- recipe(
  target_var ~ rain + snow + prec + qair + temp + patm + vapr + vpd,
  data = ddf_train) %>% 
...
conditional_dim <- CONDITIONAL_FEATURES
...
fc2 <- nn_sequential(nn_linear(in_features=64, 
                                       out_features =64), 
                              nn_relu()
                              )
    
fc3 <- nn_sequential(nn_linear(in_features =64, 
                                        out_features =16), 
                              nn_relu()
                              )
fc4 <- nn_linear(16, 1)
...
for (i in 1:length(train_sites)){
  ...
  train_list[[i]] <- site_df%>%select(-"sitename","target_var")
}
...
for (i in 1:length(train_sites)){
  site_df <- ddf_train_norm[ddf_train_norm$sitename==train_sites[i],]
  target_train_list[[i]] <- site_df%>%select("target_var")
}
...
optimizer <- optim_adam(model_condition$parameters, lr = 0.0001)
```
...
for (epoch in 1:n_epochs){
  ...
  for (i in 1:ncol(training_df)){
    ...
    x <- torch_tensor(x %>% select(-"target_var") 
                       %>%as.matrix(), 
                      dtype = torch_float()
                      )$to(device = DEVICE)
    ...
    loss <- nnf_binary_cross_entropy(y_pred,y) #changed to CE loss
