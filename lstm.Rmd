...
df_phenology <- readRDS("data/df_phenology.rds")
df_watch <- readRDS("data/df_watch.rds")
library(data.table)
library(torch)
library(tidyverse)
library(recipes)
...
#normalisaton
myrecipe_ddf <- recipe(
  target ~ rain + snow + prec + qair + temp + patm + vapr + vpd, data = ddf_train) %>% 
...
mymodule_generator_withcondition <- nn_module(
  ...
  forward = function(x,c){    
    ...
    # we concatenating the time-invariant variables to the outputs of LSTM when utilizing 
    # condition part of the model. 
